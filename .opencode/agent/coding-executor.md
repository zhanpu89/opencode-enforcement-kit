...
**子步骤 3：读设计文档**
阅读 `doc/` 下对应模块的详细设计文档和 `.opencode/rules/endpoint-lock.md`（端对齐规则）。

**子步骤 4：编码**
按 `.opencode/rules/precise-location.md` 精准定位后，严格按文档写代码。不越界、不补位、不自由发挥。
...
- 遇到端与端不一致 → 按 `.opencode/rules/endpoint-lock.md` 执行 STOP → READ → IDENTIFY → REPORT → WAIT 流程。
