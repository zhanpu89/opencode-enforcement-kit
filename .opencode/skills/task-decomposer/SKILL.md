...
### Step 7：自动评审（跨会话独立上下文）

文档生成并保存后，必须执行以下自动评审流程，不得跳过。上限 3 轮。

**第 1 轮：**

1. 收集本次写入的文档路径列表，存入变量 DOC_PATHS
2. 调用 `task` 工具启动评审子代理，参数如下：
   - description: `"评审: {文档名}"`
...
3. 从 `task` 返回消息中提取最后几行的标记：
   - `REVIEW_CONCLUSION: ✅` 或 `⚠️`
     → 运行 `bash scripts/gate.sh pass detailed`，流程完成
   - `REVIEW_CONCLUSION: ❌`
     → 提取所有 `P0_BLOCKING:` 行的内容 → 进入第 2 轮

**第 2~3 轮（仅在 P0 阻断时）：**

4. 按 `P0_BLOCKING` 行逐条修复详设文档，每条对应一个 P0 问题
5. 更新文档版本号（v1.x → v1.x+1）和变更记录
6. 返回第 2 步，用修复后的文档路径重新调用 `task`

**超限处理：**

3 轮后仍存在 P0 → 停止循环，输出以下信息，等待人工介入：

```
⛔ 经 3 轮修复仍存在 P0 阻断项，需人工介入。
评审报告见: doc/review/xxx_详细设计评审报告.md
```
