...
## 上下文记忆
init_session + search_summaries(tags="detailed", limit=3) + related_decisions(query="详设|API|接口|数据模型")
完成时 add_decision(reasoning="含被否方案和约束条件") + save_summary(next_steps="进入编码阶段")
...
| 文件 | 加载时机 | 释放时机 |
|------|---------|---------|
| `resources/glossary.md` | *首次触发时*——核心术语、LC规则体系 | *全部分段生成结束后* |
| `templates/progress-format.md` | Step 1 创建进度文档前 | *进度文档创建后* |
| `resources/layer-model.md` | Step 2 依赖排序时 | *Step 3 生成后端详设前* |
| `resources/chain-derivation.md` | Step 2.5 功能链推导时 | *Step 3 生成后端详设前* |
| `templates/backend-detailed.md` | Step 3 生成后端详设前 | *全部分段生成结束后* |
| `templates/frontend-template.md` | Step 4 前——前端/小程序模板（按端跳转对应 `##` 节） | *全部分段生成结束后* |
| `resources/frontend-guide.md` | Step 4 前——前端/小程序设计指南（按端跳转对应 `##` 节） | *全部分段生成结束后* |
| `templates/rules-template.md` | Step 5 前——编码/项目规则模板（按需跳转 `##` 节） | *生成完成后* |
| `resources/lang-engineering.md` | Step 5 生成项目规则时——语言工程规则（按 LC-001 跳转对应节） | *项目规则生成完成后* |
...
按参考文件表加载，立即写入 `_PROGRESS.md`——**这是第一个文件写入，完成后才允许 Step 2**。

### Step 2：依赖排序
按参考文件表加载。按 LC-001 分配层次（Layer 0-4），建立构建顺序。

### Step 2.5：功能链完整性推导（不可跳过）
按参考文件表加载。**7条规则全部扫描，逐模块执行**：
...
### Step 3：后端详设生成（逐文档）
按参考文件表加载。每份文档包含13节：功能描述/业务规则/OpenAPI 3.0 YAML/伪代码（含编码约束标记）/状态机与状态流转/算法/DDL/外部接口/内部接口/性能要求（含缓存失效规则）/安全要求/测试要点（含异常场景覆盖）/依赖关系。
