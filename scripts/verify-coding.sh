...
usage() {
    ...
    echo "  $0 post user-service 'biz=ok urls=ok params=ok entity=ok no-drift=yes' --fix"
    echo "       （评审修复模式，跳过前置验证检查）"
    echo ""
...
do_pre() {
    ...
    shift
    # 兼容两种传参方式：
    #   1) 空格分隔的单个字符串："a.md b.md"
    #   2) 多个独立参数：a.md b.md
    local doc_files=()
    for arg in "$@"; do
        while IFS=' ' read -ra parts; do
            for part in "${parts[@]}"; do
                [ -n "$part" ] && doc_files+=("$part")
            done
        done <<< "$arg"
    done
...
do_post() {
    ...
    # Parse optional flags and memory fields
    ...
    if [ "$fix_mode" = false ] && [ ! -f "$VERIFY_DIR/$module.ini" ]; then
        echo "❌ 未找到前置验证记录，请先运行: $0 pre $module"
        echo "   （评审修复请加 --fix 跳过前置验证）"
        exit 1
    fi
